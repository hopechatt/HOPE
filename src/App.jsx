...
/* ── fare calculation (uses real distance if provided) ── */
function calcFare(distanceMiles = 2.8, durationMins = 8, hasPet = false, extraStops = 0) {
  const base = 3.50;
  const dist = distanceMiles * 1.50;
  const dur  = durationMins  * 0.30;
  const pet  = hasPet ? 5 : 0;
  const stop = extraStops * 1.50;
...
/* ── OpenRouteService geocode + route (free, no key needed for basic use) ── */
async function geocodeAddress(address) {
  try {
    const q = encodeURIComponent(address + ', Chattanooga, TN');
...
async function getRoute(pickupCoord, dropoffCoord) {
  try {
    const r = await fetch(
      `https://router.project-osrm.org/route/v1/driving/${pickupCoord.lng},${pickupCoord.lat};${dropoffCoord.lng},${dropoffCoord.lat}?overview=full&geometries=geojson`
    );
    const d = await r.json();
    if (d.routes?.[0]) {
...
function LiveMap({ pickupCoord, dropoffCoord, routeCoords, userCoord, height = '100%' }) {
  ...
  useEffect(() => {
    if (leafRef.current) return; // already initialised
  ...
  useEffect(() => {
    ...
    const makeIcon = (color, label) => L.divIcon({
      className: '',
      html: `<div style="background:${color};width:14px;height:14px;border-radius:50%;border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,0.5)"></div>`,
      iconSize: [14, 14], iconAnchor: [7, 7],
    });

    if (userCoord) {
      layersRef.current.user = L.marker([userCoord.lat, userCoord.lng], { icon: makeIcon('#4caf50') })
        .bindPopup('You are here').addTo(map);
    }
    if (pickupCoord) {
      layersRef.current.pickup = L.marker([pickupCoord.lat, pickupCoord.lng], { icon: makeIcon('#4caf50') })
        .bindPopup('Pickup').addTo(map);
    }
    if (dropoffCoord) {
      layersRef.current.dropoff = L.marker([dropoffCoord.lat, dropoffCoord.lng], { icon: makeIcon(pink) })
        .bindPopup('Dropoff').addTo(map);
    }
    if (routeCoords?.length) {
      const latlngs = routeCoords.map(([lng, lat]) => [lat, lng]);
      layersRef.current.route = L.polyline(latlngs, { color: pink2, weight: 4, opacity: 0.85 }).addTo(map);
...
export default function App() {
  ...
  const [routeInfo,    setRouteInfo]    = useState(null);   // {distanceMiles, durationMins}
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        pos => setUserCoord({ lat: pos.coords.latitude, lng: pos.coords.longitude }),
  ...
  /* ── auto-geocode & route when both addresses filled ── */
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      if (pc && dc) {
        ...
        if (route) { setRouteCoords(route.geometry); setRouteInfo({ distanceMiles: route.distanceMiles, durationMins: route.durationMins }); }
      }
  ...
  /* ── booking ── */
  const fare = calcFare(
    routeInfo?.distanceMiles || 2.8,
    routeInfo?.durationMins  || 8,
    hasPet, stops.filter(Boolean).length
  );
  ...
  async function requestRide() {
    ...
    try {
      const res = await fetch('/.netlify/functions/create-stripe-checkout', {
        ...
        body: JSON.stringify({ amountInCents: Math.round(parseFloat(fare.total) * 100), riderEmail: session.user.email, pickupAddress: pickup, dropoffAddress: dropoff }),
      });
  ...
  if (safety) return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', padding: '24px', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ maxWidth: '480px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <button onClick={triggerSOS} style={{ background: '#c62828', color: '#fff', border: 'none', borderRadius: '16px', padding: '22px', fontWeight: '900', fontSize: '20px', cursor: 'pointer', boxShadow: '0 6px 24px rgba(198,40,40,0.6)', letterSpacing: '1px' }}>⚠️ SOS — Emergency Alert</button>
        ...
        <div style={{ background: cardBg, borderRadius: '16px', padding: '16px' }}>
          ...
          <p style={{ margin: '0 0 12px', color: '#bbb', fontSize: '13px' }}>Let a trusted contact follow your ride.</p>
        ...
        {activeRide && (
          <div style={{ background: cardBg, borderRadius: '16px', padding: '16px', border: `1px solid ${pink}` }}>
            ...
            <p style={{ margin: '0 0 3px', color: '#ccc', fontSize: '13px' }}>From: {activeRide.pickup_address}</p>
            <p style={{ margin: '0 0 12px', color: '#ccc', fontSize: '13px' }}>To: {activeRide.dropoff_address}</p>
      ...
      <div style={{ minHeight: '100vh', background: dark, fontFamily: 'system-ui,sans-serif', position: 'relative', overflow: 'hidden', height: '100vh' }}>
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 20, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'linear-gradient(to bottom,rgba(0,0,0,0.8),transparent)' }}>
          ...
          <div style={{ ...row, gap: '8px' }}>
            <button onClick={() => setSafety(true)} style={{ background: 'rgba(162,19,93,0.9)', border: 'none', color: '#fff', borderRadius: '20px', padding: '7px 14px', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>🛡️ Safety</button>
        ...
        {routeLoading && (
          <div style={{ position: 'absolute', top: activeRide ? '160px' : '70px', left: '50%', transform: 'translateX(-50%)', zIndex: 15, background: 'rgba(0,0,0,0.8)', borderRadius: '20px', padding: '8px 16px' }}>
            <p style={{ margin: 0, color: '#E0358D', fontSize: '13px', fontWeight: '700' }}>📍 Finding route...</p>
          </div>
        )}
          ...
          {!sheet && (
            <div style={{ padding: '16px 20px 28px' }}>
              ...
              <button onClick={() => { setSheet(true); setRTab('book'); }}
                style={{ ...row, gap: '12px', background: '#fff', borderRadius: '14px', padding: '16px 18px', border: 'none', cursor: 'pointer', width: '100%', marginBottom: '14px' }}>
              ...
              <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                {rTab === 'home' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div style={{ display: 'flex', gap: '10px' }}>
                      {[[history.length, 'Rides', 'rgba(162,19,93,0.25)'], ['5.0 ⭐', 'Rating', cardBg], ['✓ Active', 'Status', cardBg]].map(([v, l, bg]) => (
                    ...
                    <button onClick={triggerSOS} style={{ background: '#c62828', color: '#fff', border: 'none', borderRadius: '14px', padding: '16px', fontWeight: '900', fontSize: '16px', cursor: 'pointer', boxShadow: '0 4px 16px rgba(198,40,40,0.4)' }}>⚠️ SOS — Emergency Alert</button>
                  </div>
                )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    {routeInfo && (
                      <div style={{ ...row, gap: '12px', background: 'rgba(162,19,93,0.15)', borderRadius: '12px', padding: '10px 14px', border: `1px solid ${pink}` }}>
                        ...
                        <div>
                          <p style={{ margin: 0, color: '#fff', fontWeight: '700', fontSize: '14px' }}>{routeInfo.distanceMiles.toFixed(1)} miles · {Math.round(routeInfo.durationMins)} min</p>
                          <p style={{ margin: 0, color: '#888', fontSize: '12px' }}>Route calculated from real roads</p>
                        </div>
                      </div>
                    )}
                    ...
                    <div style={{ background: cardBg, borderRadius: '16px', padding: '16px', border: `2px solid ${pink}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        {routeInfo && <p style={{ margin: '4px 0 0', color: '#555', fontSize: '11px' }}>~{Math.round(routeInfo.durationMins)} min ETA</p>}
                      </div>
                    ...
                    <div style={{ background: cardBg, borderRadius: '14px', padding: '14px' }}>
                      ...
                      {[
                        ['Base Fare', `$${fare.base}`],
                        [`Distance (${fare.distanceMiles} mi × $1.50)`, `$${fare.dist}`],
                        [`Duration (${fare.durationMins} min × $0.30)`, `$${fare.dur}`],
                        hasPet && ['Pet Fee', '$5.00'],
                    ...
                    <button style={btn()} onClick={requestRide} disabled={loading || routeLoading}>
                      {routeLoading ? '📍 Calculating route...' : loading ? 'Processing...' : `💳 Request & Pay $${fare.total}`}
                    </button>
                  </div>
                )}
                ...
                {rTab === 'schedule' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    <div style={{ background: cardBg, borderRadius: '14px', padding: '14px', textAlign: 'center' }}><p style={{ margin: 0, color: '#555', fontSize: '13px' }}>No scheduled rides yet</p></div>
                  </div>
                )}
                ...
                {rTab === 'history' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    {history.length === 0
                      ? <div style={{ background: cardBg, borderRadius: '14px', padding: '20px', textAlign: 'center' }}><p style={{ margin: 0, color: '#555' }}>No rides yet. Book your first ride! 🌸</p></div>
                      : history.map(r => (
    ...
    return (
      <div style={{ minHeight: '100vh', background: dark, fontFamily: 'system-ui,sans-serif' }}>
        ...
        <div style={{ ...row, gap: '10px', padding: '14px 16px' }}>
          {[['⏳', stats.pending, 'Pending'], ['🚗', stats.today, 'Today'], ['✅', stats.completed, 'Done'], ['💰', `$${stats.earnings.toFixed(0)}`, 'Earned']].map(([icon, val, label]) => (
            <div key={label} style={{ background: cardBg, borderRadius: '12px', padding: '12px', flex: 1, textAlign: 'center' }}>
        ...
        <div style={{ ...row, gap: '8px', padding: '0 16px 12px', overflowX: 'auto' }}>
          {[['requests', `Requests${pendingList.length > 0 ? ` (${pendingList.length})` : ''}`], ['active', 'Active'], ['earnings', 'Earnings'], ['map', 'Live Map']].map(([t, l]) => (
        ...
        <div style={{ padding: '0 16px 32px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <>
              ...
              <div style={{ background: cardBg, borderRadius: '14px', padding: '14px' }}>
                {[['Gross Fares', `$${stats.earnings.toFixed(2)}`], ['Platform Fee (20%)', `-$${(stats.earnings * 0.20).toFixed(2)}`], ['You Keep', `$${(stats.earnings * 0.80).toFixed(2)}`]].map(([k, v], i) => (
                  <div key={k} style={{ ...row, justifyContent: 'space-between', paddingBottom: i < 2 ? '10px' : '0', borderBottom: i < 2 ? '1px solid #2a2a2a' : 'none', marginBottom: i < 2 ? '10px' : '0' }}>
                    <span style={{ color: '#888', fontSize: '13px' }}>{k}</span>
          ...
          {dTab === 'map' && (
            <>
              ...
              <p style={{ margin: 0, color: '#444', fontSize: '11px', textAlign: 'center' }}>© OpenStreetMap contributors</p>
            </>
          )}
        </div>
      </div>
